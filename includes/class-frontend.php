...
class Woo_Excel_Mng_Frontend
{
    ...
    public function __construct()
    {
        ...
        add_action('woocommerce_before_single_product', array($this, 'debug_meterage_disabled'), 1);
        
        //  add_action('woocommerce_before_add_to_cart_button', array($this, 'add_meterage_label_simple'), 5);
         
     add_filter('woocommerce_quantity_input_args', array($this, 'add_meterage_to_quantity'), 10, 2);
...
public function add_meterage_to_quantity($args, $product) {
    add_action('woocommerce_after_quantity_input_field', array($this, 'echo_meterage_label'));
    return $args;
}
 ...
 public function echo_meterage_label() {
    echo '<label class="meterage-label-right">متراژ</label>';
    ?>
    <style>
        .quantity {
            display: flex;
    ...
    <?php
        ...
        // public function add_meterage_label_simple() {
        //     echo '<label style="margin-left: 10px; display: inline-block;">متراژ</label>';
    ...
    /**
     * دیباگ برای نمایش وضعیت تیک محصولات (فقط برای ادمین)
     */
    public function debug_meterage_disabled()
    {
        ...
        // دریافت لیست محصولات غیرفعال شده
        $disabled_products = get_option('woo_excel_mng_disable_meterage_products', array());
        if (!is_array($disabled_products)) {
            $disabled_products = array();
        }

        $is_disabled = in_array($product_id, $disabled_products);
        if ($is_disabled) {
            echo '
        <style>
        .quantity {
    ...
    public function add_custom_quantity_field()
    {
        ...
        echo '<div style="clear: both;" class="custom-quantity-field woo-excel-custom-quantity-field">';
